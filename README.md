...
Images are built and tagged for a specific iRODS release. The most recent build is tested to be compatible with iRODS version `4.3.5`. Our goal is to keep up with the most recent stable release of iRODS. Once ugprading to a new major iRODS release, updates for older versions will not be made.
...
In addition to the base image, we provide the images `${VERSION}-sssd` (e.g., `4.3.5-1-sssd`) which have SSSD installed.
